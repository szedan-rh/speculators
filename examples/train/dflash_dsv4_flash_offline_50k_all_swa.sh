#!/bin/bash
# Offline DFlash Training Script for DeepSeek-V4-Flash (All-SWA + Muon)
#
# All 5 draft layers use sliding window attention (window=2048).
# This is compatible with vLLM's DFlash SWA inference support.
#
# Usage:
#   bash examples/train/dflash_dsv4_flash_offline_50k_all_swa.sh
...
CHECKPOINT_DIR="$BASE_DIR/checkpoints-all-swa-muon"
...
# Logging
LOGGER_FLAG="--logger trackio --run-name dflash-dsv4-flash-offline-50k-all-swa"
# =======================================

source "$SPECULATORS_VENV/bin/activate"

echo "=== Offline DFlash Training for DeepSeek-V4-Flash (All-SWA) ==="
echo "  Data:           $DATA_DIR"
...
CUDA_VISIBLE_DEVICES="$TRAIN_GPUS" torchrun \
    ...
    --on-missing skip \
    $LOGGER_FLAG 2>&1 | tee "$BASE_DIR/train-all-swa-muon.log"
